...
require_text docker/unraid/supervisord.conf 'PORT="3004",WORLDMONITOR_RELAY_KEY="",WORLDMONITOR_LOCAL_RELAY_KEY="%(ENV_RELAY_SHARED_SECRET)s"'
require_text docker/unraid/supervisord.conf 'WORLDMONITOR_RELAY_KEY="%(ENV_RELAY_SHARED_SECRET)s",WORLDMONITOR_LOCAL_RELAY_KEY=""'
...
require_absent tests/unraid-aio-isolation.test.sh '| grep -q'
require_absent tests/unraid-aio-readonly.test.sh '| grep -q'
require_absent tests/unraid-aio-seed.test.sh '| grep -q'
require_absent tests/unraid-aio-security.test.sh '| grep -q'
