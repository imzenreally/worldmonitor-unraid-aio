...
This fork also publishes an **[unofficial all-in-one image and Unraid deployment guide](docs/UNRAID.md)**, including a **[provider-by-provider API key guide](docs/UNRAID_API_KEYS.md)**. Beginning 2026-07-23, GitHub user `imzenreally` modified the upstream work for AIO/Unraid distribution; the dated scope and attribution are recorded in [`NOTICE`](NOTICE). It is maintained separately from, and is not endorsed by, the upstream World Monitor project.
